                                    ...
                                    <?php if (count($context['tasks']) > 0) : ?>
                                        <?php foreach ($context['tasks'] as $index => $item) : ?>
                                            <tr>
                                                <td><?= $index + 1 ?></td>
                                                <td>
                                                    <a href=<?php echo __URL__ . "index.php/task/" . $item->id ?> style="color: black; text-decoration: underline"><?= $item->title ?></a>
                                                </td>
                                                ...
                                                <td style="text-align: center">
                                                    <?php if ($item->status == 0) : ?>
                                                        <a href=<?php echo __URL__ . "index.php/editTask/" . $item->id ?> title="Редактировать" class="edit-icon"><i class="fa fa-edit"></i></a>
                                                    <?php endif; ?>
                                                </td>
                                                <td style="text-align: center">
                                                    <a href=<?php echo __URL__ . "index.php/deleteTask/" . $item->id . "/" . $item->title ?> style="color: red" title="Удалить" class="danger-icon"><i class="fa fa-trash"></i></a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
